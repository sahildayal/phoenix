...
import pytest
from phoenix.common import exceptions as common_exceptions
from phoenix.common import executors as common_executors
...
def test_re_exports_are_the_shared_objects_not_copies() -> None:
    for name in FROZEN_EXECUTOR_NAMES:
        assert getattr(executors, name) is getattr(common_executors, name)
...
def test_legacy_class_lookup_survives_either_import_order(first_import: str) -> None:
    ...
    script = textwrap.dedent(
        ...
        from phoenix.common import executors
        from phoenix.common.rate_limiters import RateLimitError as shared
...
def test_exceptions_re_export_the_shared_base() -> None:
    ...
    assert client_exceptions.PhoenixException is common_exceptions.PhoenixException

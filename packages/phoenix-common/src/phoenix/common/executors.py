...
from phoenix.common.exceptions import PhoenixException
from phoenix.common.rate_limiters import RateLimitError
...
# Compatibility shim for version skew. An `arize-phoenix-evals` released before the rate limiter
# and executors moved here defines its own `RateLimitError` and `PhoenixException` class objects,
# which the identity checks below would otherwise miss — silently demoting a provider rate limit to
# an ordinary retry (losing the concurrency back-off) and an internal error to a full retry loop.
# Resolution is deliberately lazy: a module-level import would run while this module's body is
# still executing and cycle back through the `phoenix.evals` re-export shims, and the resulting
# ImportError would be swallowed by the same `except` clause that is meant to handle a missing
# package. The modules are named as strings because this package deliberately does not depend on
# arize-phoenix-evals. All of this can go when pre-consolidation evals falls out of support at the
# next evals major; the exact version the mixed-version tests resolve lives in
# requirements/packages/phoenix-common-mixed-version.txt.
def _legacy_classes(
    module_name: str, attribute: str, current: Type[BaseException]
) -> Tuple[Type[BaseException], ...]:
    try:
        module = importlib.import_module(module_name)
    except ImportError:
        return ()
    legacy: object = getattr(module, attribute, None)
    if not isinstance(legacy, type) or not issubclass(legacy, BaseException):
        return ()
    if legacy is current:
        # A consolidated evals re-exports this very class, so there is nothing extra to check.
        return ()
    return (legacy,)


@lru_cache(maxsize=1)
def _legacy_rate_limit_errors() -> Tuple[Type[BaseException], ...]:
    return _legacy_classes("phoenix.evals.rate_limiters", "RateLimitError", RateLimitError)


@lru_cache(maxsize=1)
def _legacy_phoenix_exceptions() -> Tuple[Type[BaseException], ...]:
    return _legacy_classes("phoenix.evals.exceptions", "PhoenixException", PhoenixException)
    ...
    async def consumer(
        ...
        while True:
            ...
            try:
                ...
                is_rate_limit_error = isinstance(exc, RateLimitError) or isinstance(
                    exc, _legacy_rate_limit_errors()
                )
                is_internal_error = isinstance(exc, PhoenixException) or isinstance(
                    exc, _legacy_phoenix_exceptions()
                )

                is_phoenix_exception = is_internal_error and not is_rate_limit_error
                ...
                try:
                    for attempt in range(self.max_retries + 1):
                        ...
                        try:
                            ...
                            # No rate-limit tier here: a RateLimitError subclasses
                            # PhoenixException, so the sync path raises it immediately instead of
                            # backing off. That divergence from AsyncExecutor is longstanding and
                            # is pinned by a test rather than changed here.
                            is_phoenix_exception = isinstance(exc, PhoenixException) or isinstance(
                                exc, _legacy_phoenix_exceptions()
                            )

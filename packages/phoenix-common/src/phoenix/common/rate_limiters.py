...
def _write_to_progress_bar(message: str) -> None:
    tqdm.write(message)
...
class AdaptiveTokenBucket:
    ...
    notify (Callable[[str], None]): Where throttling messages go when verbose is enabled.
    """
    ...
    def __init__(
        ...
        notify: Callable[[str], None] = _write_to_progress_bar,
    ):

"""Resolve project evaluator criteria into fingerprint inputs, and record that a
criteria has produced work.
...
async def stamp_work_materialized(session: AsyncSession, criteria_id: int) -> None:
    """Record that this criteria has produced evaluation work, once and for good.

    The stamp is what the evaluationTarget lock reads, so it must survive the work rows
    that retention will eventually delete. Both materializers call it at their insert
    chokepoint. updated_at is carried forward explicitly: this is materializer
    bookkeeping, not a configuration change, and ProjectEvaluator.updatedAt is a live
    GraphQL field.
    """
    await session.execute(
